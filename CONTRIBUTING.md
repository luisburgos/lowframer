# Contributing to lowframer

## What this is

Lowframer is a Flutter package: a kit of primitives for drawing low-fidelity
wireframe art — miniature, theme-aware UI illustrations built from code.
...
```
feat(scribble): add a dotted style
fix(palette): derive the backdrop from onSurface
```
...
| the `lowframer: ^X.Y.Z` install snippet in `README.md` | everyone reading the package's front page |
